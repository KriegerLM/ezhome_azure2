...
page import="java.util.Objects" %> <%@page contentType="text/html"
pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="es">
  <head>
    ...
    <main id="product-container" class="product-container2">
      ...
      <div class="Container_form">
        ...
        <section class="form-section">
          ...
          <form
            id="catalog-form"
            action="Catalogo_Admin_Modif_B.jsp?busqueda=<%=busqueda%>"
            method="POST"
          >
            <input
              type="text"
            ...
            <div class="form-actions">
              <button type="submit" name="botons" id="subir" disabled>
                Llena todos los campos para crear
              </button>
              <button
                type="submit"
                name="botons"
      ...
      <section class="catalog-section">
        ...
        <div id="product-catalog" class="product-catalog">
          ...
          <% Base bd = new Base(); try { bd.conectar(); String strQry = "select
          imagen, nombre, categoria, descripcion, precio, existencias, id_regalo
          from producto where nombre like '%"+busqueda+"%';"; ResultSet rs =
          null; rs = bd.consulta(strQry); while (rs.next()) { %>
          ...
          <div class="product-card">
            ...
            <p class="product-category">
              <strong>Categoría:</strong> <%=rs.getString(3)%>
            </p>
            <p class="product-description"><%=rs.getString(4)%></p>
            <p class="product-description">
              <strong>Existencias:</strong><%=rs.getString(6)%>
            </p>
            <p class="product-price">
              <strong>Precio:</strong> $<%=rs.getString(5)%>
            </p>
            <button
              class="add-to-cart-btn"
              id="button<%=rs.getString(7)%>"
              onclick="fillForm<%=rs.getString(7)%>(this);verificarCampos()"
            >
              Seleccionar
            </button>
          </div>
          ...
          <% } bd.cierraConexion(); } catch (Exception ex) {
          System.out.println(ex.getMessage()); //out.print(ex.getMessage()); }
          %>
          <script>
            ...
            document
              .querySelectorAll('button[id^="button"]')
              .forEach((button) => {
              ...
              if (idproducto.value.trim() === "") {
                ...
                if (!todosLlenos === true) {
                  ...
                  botonEnviar.innerText =
                    "Llena todos los campos para modificar";
